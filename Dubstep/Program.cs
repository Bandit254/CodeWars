...
namespace Dubstep
{
    class Program
    {
        ...
        {
            Console.WriteLine("Decode dubstep lyrics.\n");
            Console.WriteLine($"The original lyrics from the dubstep song 'WUBWEWUBAREWUBWUBTHEWUBCHAMPIONSWUBMYWUBFRIENDWUB' are: {SongDecoder("WUBWEWUBAREWUBWUBTHEWUBCHAMPIONSWUBMYWUBFRIENDWUB")}.\n");
            Console.WriteLine($"The original lyrics from the dubstep song 'WUBWUBABCWUB' are: {SongDecoder("WUBWUBABCWUB")}.\n");
            Console.WriteLine($"The original lyrics from the dubstep song 'RWUBWUBWUBLWUB' are: {SongDecoder("RWUBWUBWUBLWUB")}.\n");
        }
        public static string SongDecoder(string input)
        {
            string originalLyrics = "";
            string[] lyricArray = input.Split("WUB");
            for (int i = 0; i < lyricArray.Length; i++)
            {
                if (lyricArray[i]!="")
                {
                    originalLyrics += lyricArray[i]+" ";
                }
            }
            originalLyrics=originalLyrics.Trim();
